...
kdtreeToString :: KDString -> String
kdtreeToString (KLeaf t xs ys) = show t ++ show xs ++ show ys ++ show "\n"
kdtreeToString (KNode cut lt rt) =
    -- get user to supply the overlap ratio
    case cut of Horizontal _ -> kdtreeToString lt ++ " : Horizontally composed with " ++ kdtreeToString rt ++ show "\n"
                Vertical     -> kdtreeToString lt ++ " : Vertically composed with " ++ kdtreeToString rt ++ show "\n"
...
main = do
    putStrLn (kdtreeToString mytree)